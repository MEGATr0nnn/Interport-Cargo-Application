            ...
            try
            {
                ...
                string createTableQuery = @"
                        CREATE TABLE IF NOT EXISTS user (
                            ...
                            user_PhoneNumber INTEGER NOT NULL
                        )";
        ...
        public void insertNewUser(User user)
        {
            try
            {
                ...
                string insertUserQuery = @"
                    INSERT INTO user (user_First_Name, user_Last_Name, user_Email, user_PhoneNumber) 
                    VALUES (@user_First_Name, @user_Last_Name, @user_Email, @user_PhoneNumber)";
                ...
                using (SQLiteCommand command = new SQLiteCommand(insertUserQuery, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@user_PhoneNumber", user.getPhoneNumber());
                    command.ExecuteNonQuery();
                }
            }
            catch (Exception e)
                ...
                using (SQLiteCommand command = new SQLiteCommand(selectQuery, connection))
                {
                    ...
                    using (SQLiteDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            int phoneNumber = reader.GetInt32(reader.GetOrdinal("user_PhoneNumber"));
